...
- **커스텀 액션(Custom Action)**: 표준 CRUD 작업(GET, POST, PUT, DELETE) 외에 특정 비즈니스 로직이 필요한 경우, `{resource_name}:{action}` 형태로 표기합니다.
  - 예시: `POST /users:bulk` (사용자 일괄 생성), `POST /orders/{id}:cancel` (주문 취소), `GET /reports:export` (보고서 내보내기).
  - 형식: 리소스 경로 뒤에 콜론(`:`)을 붙이고 액션명을 소문자로 작성합니다.
  - 목적: RESTful 원칙을 최대한 유지하면서도, 복잡한 비즈니스 로직이나 대량 작업 등 표준 메서드로 표현하기 어려운 작업을 명확히 구분합니다.
  - 주의: 커스텀 액션은 최소한으로 사용하며, 가능한 한 리소스 기반 설계를 우선합니다. 과도한 커스텀 액션 추가는 API 일관성을 해치므로 신중히 도입하세요.
